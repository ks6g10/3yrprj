...
\hline\end{tabular}
\end{table}
In the following subsections, we describe the DP algorithm which we build upon  and explain how we adapt the data structures and the sub-routines of the algorithm  to create a new algorithm that can run on the GPU. 
...
\end{algorithmic}
\end{algorithm}
Having described the DP algorithm, we next elaborate on how the data structure we use to extend DP into GPU-CSG.

\subsection{The Data Structure} %done
...
It does so by taking a count as input representing which nth splitting should be created, finds the index of the counts  set bits, and finally left shifts each bit with the value in the shift array its index reference to,  this is also illustradet in figure \ref{fig:howitworks}. Then to generate the next splitting, a call to nextSplit in algorithm \ref{alg:nextsplit} is needed which works through a recurrence relation to generate the next splitting.
