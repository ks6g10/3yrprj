...
\subsection{Algorithm}\label{algorithm} %not done nein
...
\begin{algorithmic}[1]
\IF{$tid = 0$}
  \STATE $conf_0 := C_0$ \hfill Thread 0 sets the first coalition from input
  \FOR {$i := 1$ to $confpkernel$}
    \STATE $conf_i := nextCoalition(conf_{i-1})$ \hfill Generate the next coalitions to evaluate
  \ENDFOR \hfill Checkpoint 1
\ENDIF
\IF{$tid < confpkernel$}
  \STATE $initShift(conf_{tid},tid)$ \hfill Initialise the shift array
\ENDIF
\hfill Checkpoint 2
\FOR{$x := 0$ to $confpkernel$}\label{lst:line:bigfetch}
...
\hfill Checkpoint 3
  \FOR{$z := 0$ to $nparallelconf$}
    \IF{$conf_{z+x} \geq maxval$}\label{lst:line:gotoif}
      \STATE goto line \ref{lst:line:reductionstart} \hfill The coalition is outside the range of the calculation
    \ENDIF
    \STATE $C := initialSplit(\psi,conf_{z+x})$ \hfill Generate the $\psi$th splitting from a coalition 
    \STATE $\upsilon_{0,z} := f(conf_{z+x}\backslash C)+f(C)$ \hfill Fetch the splittings value
    \STATE $C := nextSplit(C)$ \hfill Generate the next splitting of the coalition
    \STATE $\upsilon_{1,z} := f(conf_{z+x}\backslash C)+f(C)$ \hfill Fetch the splittings value
